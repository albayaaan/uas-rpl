    ...
    <main id="main">
        ...
        <section id="form" class="form">
            <div class="container">
                ...
                <h3 class="text-center">Pendaftaran Webinar</h3>
                <form action="daftar_webinar.php" method="post" enctype="multipart/form-data">
                    <div class="row mt-3">
                        <div class="col">
                            <select class="form-select" aria-label="Default select example" name="webinar" required>
                                <option value="">--</option>
                                <?php
                                include "connect.php";
                                $id = $_GET["id"];
                                ...
                                ?>
                                    <option value="<?= $row['id'] ?>" <?= $row['id'] == $id ? "selected" : "" ?>><?= $row['topik'] ?> - Rp <?= $row['harga'] > 1000 ? $row['harga'] / 1000 : $row['harga'] ?>.000</option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-4">
                            <div class="form-floating">
                                <select class="form-select" aria-label="Default select example" name="pembayaran" required>
                                    ...
                                    while ($row = mysqli_fetch_array($hasil)) {
                                    ?>
                                        <option value="<?= $row['id'] ?>">Transfer <?= $row['transfer'] ?> - <?= $row['tujuan'] ?></option>
                                    <?php } ?>
                                </select>
